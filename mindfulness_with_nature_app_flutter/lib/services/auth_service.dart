...
class AuthService with ChangeNotifier {
  ...
  // Cached stream — created once so StreamBuilder never resets to waiting
  // when AuthService calls notifyListeners().
  late final Stream<app_model.User?> _cachedAuthStream;
  ...
  Stream<app_model.User?> get authStateChanges => _cachedAuthStream;

  Future<app_model.User?> _mapFirebaseUser(fb.User? firebaseUser) async {
  ...
  Future<app_model.User> _getUserFromFirebaseUser(fb.User firebaseUser) async {
    try {
      ...
      // Firestore doc missing — build a fallback from Firebase Auth data so
      // an authenticated user is never treated as unauthenticated.
  ...
  app_model.User _buildFallbackUser(fb.User firebaseUser) {
